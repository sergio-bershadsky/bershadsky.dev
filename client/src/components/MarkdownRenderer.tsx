...
// Part 6: Hooks Diagrams
const SkillsVsHooksDiagram = () => (
  <div className="my-8 border border-primary/30 rounded-lg bg-black/40 p-5">
    <div className="text-sm font-mono text-primary mb-4 flex justify-between">
...
const HooksFolderStructureDiagram = () => {
  const folders = [
...
const CyberCodeBlock = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  if (isAsciiDiagram) {
    ...
    // Part 6: Hooks patterns
    const isSkillsVsHooks = codeContent.includes('SKILLS') && codeContent.includes('HOOKS') && codeContent.includes('You trigger them') && codeContent.includes('They trigger themselves');
    const isHookEventsTable = codeContent.includes('EVENT') && codeContent.includes('WHEN IT HAPPENS') && codeContent.includes('Session Start') && codeContent.includes('File Created');
    const isHookTimeline = codeContent.includes('SESSION START') && codeContent.includes('BEFORE TOOL USE') && codeContent.includes('AFTER TOOL USE') && codeContent.includes('SESSION END');
    const isHookFlowDiagram = codeContent.includes('EVENT HAPPENS') && codeContent.includes('HOOK CHECKS') && codeContent.includes('CONDITION MET') && codeContent.includes('CONDITION NOT MET');
    ...
    if (isHookEventsTable) {
      return <HookEventsTable />;
    }
    
    if (isHookTimeline) {
      return <HookTimelineDiagram />;
    }
    
    if (isHookFlowDiagram) {
      return <HookFlowDiagram />;
    }
    
    if (isGoodFirstHooks) {
      return <GoodFirstHooksDiagram />;
    }
    
    if (isHooksFolderStructure) {
      return <HooksFolderStructureDiagram />;
    }
    
    if (isHookSpectrum) {
      return <HookSpectrumDiagram />;
    }
